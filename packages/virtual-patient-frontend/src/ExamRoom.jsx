...
function ExamRoomModel() {
  ...
  const { scene } = useGLTF("https://storage.googleapis.com/vp-model-storage/examroomclean.glb");
  
  useEffect(() => {
    if (scene && roomRef.current) {
      console.log("Setting up exam room model");
      
      // Clone the scene to avoid mutation issues
      const roomScene = scene.clone();
      
      // Adjust the exam room position and scale
